# -*- coding: utf-8 -*-
from flask import Blueprint, render_template, request, url_for, redirect, current_app, flash, abort, make_response

from mybluelog.forms import CommentForm, AdminCommentForm
from mybluelog.models import Post, Category, Comment
from mybluelog.extensions import db
from mybluelog.emails import send_new_comment_email, send_new_reply_email
...
def index(page):
    per_page = current_app.config['MYBLUELOG_POST_PER_PAGE']
...
def show_post(post_id):
    ...
    if current_user.is_authenticated:
        ...
        form.email.data = current_app.config['MYBLUELOG_EMAIL']
...
def change_theme(theme_name):
    if theme_name not in current_app.config['MYBLUELOG_THEMES'].keys():
        abort(404)
